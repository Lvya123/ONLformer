...
# Clear cache and print initial GPU memory
clear_gpu_cache()
print_gpu_memory("Before test")
...
for ii, data_val in enumerate(val_loader, 1):
    target_img = img_as_ubyte(data_val[0].numpy().squeeze().transpose((1, 2, 0)))
    input_full = data_val[1].cuda()
    
    original_h, original_w = input_full.shape[2], input_full.shape[3]
    print(f"\nImage {ii}: Original size = {original_h} x {original_w}")
    
    # Crop to 256x256 top-left corner
    crop_size = 256
    crop_h = min(crop_size, original_h)
    crop_w = min(crop_size, original_w)
    input_cropped = input_full[:, :, :crop_h, :crop_w]
    print(f"Cropped to: {crop_h} x {crop_w}")
    
    print_gpu_memory("After crop")
    
    with torch.no_grad():
        restored = model_restoration(input_cropped)
    
    restored = restored[0]
    restored = torch.clamp(restored, 0, 1)
    restored = restored[:, :crop_h, :crop_w]  # Ensure same size as cropped input
    restored_img = img_as_ubyte(restored.cpu().numpy().squeeze().transpose((1, 2, 0)))
    
    print_gpu_memory("After forward")
    
    # Also crop target image to same 256x256 top-left region for fair comparison
    target_img_cropped = target_img[:crop_h, :crop_w, :]
    
    # Calculate PSNR
    psnr_val = PSNR(restored_img, target_img_cropped)
    psnr_val_rgb.append(psnr_val)
    
    ######### release testing gpu memory ##########
    del restored, input_full, input_cropped
    clear_gpu_cache()
    print_gpu_memory("After cleanup")
    
    log('%-6s \t PSNR: %f' % (data_val[2][0], psnr_val), 
        os.path.join(log_dir, 'val_' + str(val_epoch) + '_256.txt'), P=True)
    
    if opt['VAL']['SAVE_IMG']:
        save_img_path = os.path.join(result_dir, 'epoch_' + str(val_epoch) + '_256')
        file_name = data_val[2][0] + '_restored_256.png'
        utils.mkdir(save_img_path)
        utils.save_img(os.path.join(save_img_path, file_name), restored_img)
    
    # Optional: process only first few images for quick test
    # if ii >= 5:
    #     break

avg_psnr = sum(psnr_val_rgb) / ii
print(f"\n{'='*50}")
print(f"Total images processed: {ii}")
print(f"Average PSNR (256x256 crop): {avg_psnr:.4f} dB")
print(f"{'='*50}")
log('total images = %d \t avg_psnr = %f' % (ii, avg_psnr), 
    os.path.join(log_dir, 'val_' + str(val_epoch) + '_256.txt'), P=True)